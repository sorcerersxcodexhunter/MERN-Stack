...
import { user_api_key } from './util.js';
import { setJob } from '../../redux/feature/AuthSlice.jsx';

function ModeleditJobs({ show, onHide, onJobPosted, jobToEdit = null }) {
  ...
  useEffect(() => {
    if (jobToEdit) {
      setJobData({
        ...
        technology: Array.isArray(jobToEdit.technology) 
          ? jobToEdit.technology.join(', ') 
          : jobToEdit.technology || '',
        details: jobToEdit.Details?.discription || jobToEdit.details || ''
      });
    } else {
      setJobData({
        title: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        // Update existing job
        response = await axios.put(`${user_api_key}/jobs/update-job/${jobToEdit._id}`, jobData, {
          headers: { 'Content-Type': 'application/json' },
          withCredentials: true
        });
      } else {
        // Create new job
        response = await axios.post(`${user_api_key}/jobs/postjob`, jobData, {
          headers: { 'Content-Type': 'application/json' },
          withCredentials: true
        });
      }
      ...
      if (response.data.status === 'success') {
        ...
        toast.error(response.data.message || (isEditing ? "Failed to update job" : "Failed to post job"));
      }
      
    } catch (error) {
      toast.error(error.response?.data?.message || `An error occurred while ${isEditing ? 'updating' : 'posting'} the job. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{isEditing ? 'Edit Job' : 'Post a New Job'}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
